...
public class FuncionPelicula extends ElementoComercial<TipoFormato>{
    ...
    public FuncionPelicula(String codigo, String nombre, TipoFormato categoria, double precioBase, int sala, int duracionMinutos, int capacidadTotal) {
    ...
    public boolean venderBoletos(int cantidad){
        if (cantidad>0 && cantidad <= getAsientosOcupados()){
    ...
    public String getDetalle(){
        return String.format("Codigo: %s\n Pelicula: %s\n Formato: %s\n Sala: %d\n Duracion: %d minutos\n Butacas: %d / %d\n Precio: $%.2f", codigo, nombre, categoria, sala, duracionMinutos, getAsientosDisponibles(),capacidadTotal, calcularPrecio());
    }
}
